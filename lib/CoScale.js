...
doHttpRequest = function (method, apiPath, coscaleMessage, callback, errCallback) {
    ...
    req = protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            if (res.statusCode === 401) { //Authentication failure
                ...
                try {
                    var parsedResBody = JSON.parse(body.join(''));
                    callback(res.statusCode, parsedResBody);
                } catch (err){
                    util.log("Error occurred while parsing the response body: " + err.stack);
                    req.emit('error', err);
                }
            }
        });
...
var postMetricValuesToApi = function (Data, callback) {
    ...
    doHttpRequest('POST', location, data, callback, stopFlush);
};
...
// stopFlush is used to end the flushing proccess
var stopFlush = function () {
